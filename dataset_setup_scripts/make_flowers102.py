...
# read the txt file
txt_train = os.path.join(dataset_path, "train.txt")
txt_val = os.path.join(dataset_path, "valid.txt")
txt_test = os.path.join(dataset_path, "test.txt")
...
with open(txt_train) as f:
  for line in f:
    label = line.split()[-1]
    name = line[:-(len(label) + 2)]
    category = int(label) + 1
    original_path = '%s/%s' % (dataset_path, name)
    target_path = '%s/train/%d/%s' % (dataset_path, category, name.replace("jpg/", ""))
    shutil.move(original_path, target_path)


with open(txt_val) as f:
  for line in f:
    label = line.split()[-1]
    name = line[:-(len(label) + 2)]
    category = int(label) + 1
...
with open(txt_test) as f:
  for line in f:
    label = line.split()[-1]
    name = line[:-(len(label) + 2)]
    category = int(label) + 1
    original_path = '%s/%s' % (dataset_path, name)
    target_path = '%s/val/%d/%s' % (dataset_path, category, name.replace("jpg/", ""))
    shutil.move(original_path, target_path)
